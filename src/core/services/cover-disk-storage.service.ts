...
export type CoverCachePartitions = Partial<Record<CoverStorageKey, number>>;
 ...
 */
const PARTITION_WEIGHTS: Record<CoverStorageKey, number> = {
  live: 0.3,
  requested: 0.15,
  played: 0.25,
  search: 0.3,
 ...
 */
function partitionCaps(
  maxBytes: number,
  custom: CoverCachePartitions | null | undefined,
): Record<CoverStorageKey, number> | null {
  if (maxBytes <= 0) return null;

  const caps: Record<CoverStorageKey, number> = {
    live: 0,
    requested: 0,
    played: 0,
    search: 0,
  ...
  if (uncappedWeight > 0) {
    ...
    for (const key of CATEGORY_ORDER) {
      if (custom?.[key] == null) {
        caps[key] = Math.round((leftover * PARTITION_WEIGHTS[key]) / uncappedWeight);
      }
    }
  }
  ...
  async trim(
    ...
    return this.enqueue(async () => {
      ...
      const result = await this.evictPartitions(
        groups,
        statByUrl,
        partitionCaps(maxBytes, partitions),
      );
