...
function timerPrompt($startTimer, $msg=''){
    //echo "[".$msg."] ".round(microtime(true) - $startTimer, 4)." sec\n";
}
...
timerPrompt($startTimer, "checagem inicial l108");

$musicdata = json_decode(file_get_contents('https://cast.animu.moe:8999/api/v2/music/' . $musicid . '/?format=json'), true);
...
timerPrompt($startTimer, "checagem dados l122");
...
timerPrompt($startTimer, "checagem dj-prog l133");
...
timerPrompt($startTimer, "checagem ip l142");
//Atualiza os pedidos antigos na DB
$updateoldnew = mysqli_query($cone, "UPDATE sp_queue SET attempt = 7 WHERE attempt IN (1,2,3) AND timestrike < (NOW() - INTERVAL 90 MINUTE);");
timerPrompt($startTimer, "checagem update l145");
...
;
timerPrompt($startTimer, "checagem 3ip l156");
//Check 3 strike (mas para ids discord)

$check_discordid = mysqli_query($cone, "SELECT discord FROM sp_queue WHERE discord = '$id' AND attempt < 4 AND timestrike > (NOW() - INTERVAL 90 MINUTE) ");
...
timerPrompt($startTimer, "checagem 3discord l166");
...
timerPrompt($startTimer, "checagem ped 24h l185");
...
timerPrompt($startTimer, "checagem anime l1198");
//Checa artista

if ($pos = strpos($songtitle, '|')) {
    $checartist = mysqli_query($cone, 'SELECT artist FROM sp_queue WHERE artist LIKE "%' . $artist . '" AND timestrike > (NOW() - INTERVAL 90 MINUTE)');
    
    $checartist2 = mysqli_query($cone, 'SELECT artist FROM sp_queue WHERE artist = "' . $artist . '" AND timestrike > (NOW() - INTERVAL 90 MINUTE)');
    
    $rownumber = mysqli_num_rows($checartist);
    $rownumber2 = mysqli_num_rows($checartist);
    if ($rownumber > 1 || $rownumber2 > 1) {
...
timerPrompt($startTimer, "checagem artista l226");
...
*/
timerPrompt($startTimer, "checagem cover l273");
//Checa se a Haru(AutoDJ) já tocou a musica nas ultimas (aproximadamente) 24 horas

$hispat = '/pedido/mi';
$musichistory = json_decode(file_get_contents('https://cast.animu.com.br:8999/api/v2/history/?limit=50&offset=0&server=1&format=json'), true);
timerPrompt($startTimer, "checagem musica tocada 24h l278");
for ($i = 0; $i < 50; $i++) {
...
timerPrompt($startTimer, "coverblock e harublock l298");    

// Uma mistura entre a harublock e coverblock (basicamente para não pedirem covers de musgas que a haruka ja tocou)

/*
for ($i = 0; $i < 50; $i++) {
   $haruexplode = explode("|", $musichistory['results'][$i]['title']);
   
   if(!preg_match($hispat, $musichistory['results'][$i]['metadata'])){
   if(isset($haruexplode[0])){
   $checaharu =  mysqli_query($cone,"SELECT music FROM sp_queue WHERE music LIKE '%".$haruexplode[0]."%' AND timestrike > (NOW() - INTERVAL 180 MINUTE)");
   if( mysqli_num_rows($checaharu) > 0){
     die("harublock");
}
   } else {
       $checaharu =  mysqli_query($cone,"SELECT music FROM sp_queue WHERE music LIKE '%".$musichistory['results'][$i]['title']."%' AND timestrike > (NOW() - INTERVAL 180 MINUTE)");
        if( mysqli_num_rows($checaharu) > 0){
     die("harublock");
}
}
...
;
timerPrompt($startTimer, "rep ip l347");    
...
curl_setopt($chpedi, CURLOPT_SSL_VERIFYHOST, 1);
curl_setopt($chpedi, CURLOPT_POST, false);
curl_setopt($chpedi, CURLOPT_URL, 'https://cast.animu.com.br/api/playrequest/add/?server=1&allmusic=' . $musicid . '&person=' . $nomenickSemTreta . '&message=' . "" . '&timeoutIP=0&timeoutTrack=0');
curl_setopt($chpedi, CURLOPT_HEADER, 0);
curl_setopt($chpedi, CURLOPT_RETURNTRANSFER, true);

$result = curl_exec($chpedi);
timerPrompt($startTimer, "envio l367");    
...
if (json_encode($result) == '"{\"status\": \"success\"}"') {
    ...
    //Fim de escrita
//echo "okidoki done";
timerPrompt($startTimer, "registro sucesso ip l411");    
        ...
        "embeds" => [
            [
                ...
                "fields" => [
                    ...
                    [
                        "name" => "Recado", "value" => "**YOUSORO!**"
                    ]
                ],
                "thumbnail" => ["url" => "$coverart_link"],
                "footer" => ["text" => "(🌐) Pedido Nº " . $result_count['COUNT(*)'] . " | " . '@' . date('B')
                ]
            ]
        ]
    ]);
...
timerPrompt($startTimer, "envio discord - fim l588");    
?>